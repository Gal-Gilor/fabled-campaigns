...
class ImagenService {
  ...
  /**
   * Initialize the service with proper credential handling
   * @private
   */
  async _initialize() {
    ...
    try {
      ...
      // Set up authentication by writing credentials to a temporary file
      // Use secure random filename to prevent predictable file paths
      const randomString = crypto.randomBytes(16).toString('hex');
      credentialsPath = path.join(os.tmpdir(), `service-account-${randomString}.json`);
      
      // Write credentials with secure file permissions (only readable by owner)
      ...
      if (credentialsPath) {
        try {
          ...
          console.warn('Could not clean up temporary credentials file:', cleanupError.message);
        }
      }
    }
  }
