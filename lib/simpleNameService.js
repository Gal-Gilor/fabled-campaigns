...
SimpleNameService.prototype.validateParams = function(type, params) {
  ...
  // Validate type
  var validTypes = ['character', 'place', 'organization', 'custom'];
  ...
  if (type === 'character') {
    if (params.race && !this.RACE_NAMES[params.race.toLowerCase()]) {
      errors.push('Unknown race: ' + params.race + '. Available races: ' + Object.keys(this.RACE_NAMES).join(', '));
    }
  ...
  if (type === 'place') {
    if (params.terrain && !this.TERRAIN_ELEMENTS[params.terrain.toLowerCase()]) {
      errors.push('Unknown terrain: ' + params.terrain + '. Available terrains: ' + Object.keys(this.TERRAIN_ELEMENTS).join(', '));
    }
    if (params.type && !this.PLACE_TYPES[params.type.toLowerCase()]) {
      errors.push('Unknown place type: ' + params.type + '. Available types: ' + Object.keys(this.PLACE_TYPES).join(', '));
    }
  }
...
SimpleNameService.prototype.generateNames = function(type, params) {
  ...
  try {
    // Validate inputs
    var validationErrors = this.validateParams(type, params);
    ...
    var count = Math.min(params.count || 5, 50); // Cap at 50 names
    var names = [];
    var attempts = 0;
    var maxAttempts = count * 10; // Allow some retries for uniqueness
    
    while (names.length < count && attempts < maxAttempts) {
      var name = this.generateRandomName(type, params);
...
SimpleNameService.prototype.addToRecentNames = function(name) {
  ...
  if (this.recentNames.size > this.maxRecentNames) {
    var oldNames = Array.from(this.recentNames).slice(0, this.maxRecentNames / 2);
...
SimpleNameService.prototype.generateFallbackNames = function(type, params) {
  var result = this.generateNames(type, params);
  return {
    success: false,
    names: result.names,
...
SimpleNameService.prototype.generateRandomCharacterName = function(params) {
  var race = params.race || 'human';
  var gender = params.gender || 'neutral';
  
  var defaultRace = this.RACE_NAMES.human;
  var raceData = this.RACE_NAMES[race.toLowerCase()] || defaultRace;
  var namePool = raceData[gender] || raceData.neutral || defaultRace.neutral;
  var surnamePool = raceData.surnames || defaultRace.surnames;
  
  // Generate using one of several patterns
  var patternIndex = Math.floor(Math.random() * 3);
  ...
  switch (patternIndex) {
    ...
    case 1: // Sometimes use nicknames (10% chance)
      if (Math.random() > 0.9) {
        firstName = namePool[Math.floor(Math.random() * namePool.length)];
        var nicknames = ['the Bold', 'the Wise', 'the Swift', 'the Brave', 'the Kind', 'the Fierce', 'the Quiet'];
    ...
    case 2: // Cross-gender names occasionally (5% chance for neutral pool)
      if (gender === 'neutral' && Math.random() > 0.95) {
        var allGenders = ['male', 'female', 'neutral'];
        var randomGender = allGenders[Math.floor(Math.random() * allGenders.length)];
        var crossNamePool = raceData[randomGender] || namePool;
