...
const { PLACE_TYPES } = require('../nameConstants');
const { TERRAIN_ELEMENTS } = require('../../js/constants');
...
const VALIDATION_RULES = {
  ...
  DETAIL_LEVEL: {
    required: false,
    allowedValues: ['detail-high', 'detail-low'],
    default: 'detail-low'
  },
  GENERATION_MODE: {
    required: false,
...
function validateString(value, rules, fieldName) {
  ...
  if (rules.allowedValues) {
    const allowed =
      typeof rules.allowedValues === 'function' ? rules.allowedValues() : rules.allowedValues;
...
function validateMapParams(params) {
  ...
  const descValidation = validateString(
    params.description,
    VALIDATION_RULES.MAP_DESCRIPTION,
    'description'
  );
  ...
  if (params.setting) {
    ...
    sanitized.setting = null;
  }

  // Validate detailLevel (optional - only set if explicitly provided)
  const detailLevelValidation = validateString(params.detailLevel, VALIDATION_RULES.DETAIL_LEVEL, 'detailLevel');
  if (!detailLevelValidation.valid) {
    errors.push(...detailLevelValidation.errors);
  } else {
    sanitized.detailLevel = params.detailLevel || null; // Don't force default for terrain/setting tabs
  }

  // Validate generation mode
  const modeValidation = validateString(
    params.generationMode,
    VALIDATION_RULES.GENERATION_MODE,
    'generationMode'
  );
...
function validateNameParams(params) {
  ...
  const descValidation = validateString(
    params.description,
    VALIDATION_RULES.NAME_DESCRIPTION,
    'description'
  );
  if (!descValidation.valid) {
    errors.push(...descValidation.errors);
