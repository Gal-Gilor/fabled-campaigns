...
const getGoogleCredentials = () => {
  ...
  throw new Error('Either GOOGLE_SERVICE_ACCOUNT_KEY (base64) or GOOGLE_APPLICATION_CREDENTIALS (file path) environment variable is required');
};
...
const validateEnvironment = () => {
  ...
  if (!process.env.GOOGLE_SERVICE_ACCOUNT_KEY && !process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    throw new Error('Either GOOGLE_SERVICE_ACCOUNT_KEY (base64) or GOOGLE_APPLICATION_CREDENTIALS (file path) must be provided');
  }
};
