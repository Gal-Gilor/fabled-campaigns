...
const { generateEnhancedPrompt } = require('./services/promptEnhancementService');
...
class ImagenService {
  ...
  async generateMapImage(params) {
    try {
      ...
      // Try to generate enhanced prompt using Gemini, fallback to simple prompt
      let prompt;
      ...
      try {
        ...
        const enhancementResult = await generateEnhancedPrompt(enhancementParams);

        if (enhancementResult.success && enhancementResult.enhancedPrompt) {
          prompt = enhancementResult.enhancedPrompt;
          enhancementUsed = true;
          console.log('Prompt enhancement succeeded');
        } else {
          throw new Error('Enhancement failed: ' + (enhancementResult.error || 'Unknown error'));
        }
      } catch (enhancementError) {
        console.warn('Prompt enhancement failed, using fallback:', enhancementError.message);
        prompt = this.buildMapPrompt(params);
        enhancementUsed = false;
      }

      const useCase =
        params.generationMode === 'quick' ? 'image-generation-quick' : 'image-generation-detailed';
      const model = getRecommendedModel(useCase);
      const config = getRecommendedConfig(useCase);

      console.log('Generating image with model:', model, '| Enhancement:', enhancementUsed);
      ...
      console.log('Image generated successfully:', imageBytes.length, 'bytes');
      ...
      return {
        ...
        metadata: {
          model: model,
          prompt: prompt,
          enhancedPromptDescription: prompt, // Use the enhanced/fallback prompt as the description
          parameters: params,
  ...
  buildMapPrompt(params) {
    const { name, description, terrain, setting, detailLevel } = params;
    ...
    if (terrain && setting) {
      ...
      prompt += `MUST be top-down perspective showing a ${setting} setting. `;
    } else if (terrain) {
      prompt += `MUST be top-down perspective showing a ${terrain} terrain. `;
    }
    ...
    // Add detail level instructions if provided
    if (detailLevel) {
      if (detailLevel === 'detail-high') {
        prompt += 'DETAIL LEVEL: Highly detailed view of a small area. Show individual features, small objects, and fine details. Each grid square should represent approximately 5 feet. ';
      } else if (detailLevel === 'detail-low') {
        prompt += 'DETAIL LEVEL: Wide view of a large area. Show major landmarks, large features, and expansive layout. Each grid square should represent a much larger area. Zoomed out perspective. ';
      }
    }
    ...
    prompt +=
      'Format: Square aspect ratio, high resolution, suitable for printing and digital use.';
